...
async fn task_camera_capture(rec: RecordingStream) {
    ...
    */
    ...
    //NOTE! info() and metadata formatting is currently busted on v0.4.0 libcamera
    let mut framenum = 0;
    ...
    loop {
        ...
        let mut req = rx
            .recv_timeout(Duration::from_millis(1000).into())
...
#[embassy_executor::task]
async fn task_ticker(rec: RecordingStream) {
    loop {
        info!("tick");
        rec.log(
            "logs",
            &rerun::TextLog::new("tick!").with_level(rerun::TextLogLevel::INFO),
        );
...
#[embassy_executor::task]
async fn task_beeper(rec: RecordingStream) {
    loop {
        info!("beep");
        rec.log(
            "logs",
            &rerun::TextLog::new("beep!").with_level(rerun::TextLogLevel::INFO),
        );
...
async fn main(spawner: Spawner) {
    ...
    spawner.spawn(task_ticker(rec.clone())).unwrap();
    spawner.spawn(task_beeper(rec.clone())).unwrap();
    spawner.spawn(task_camera_capture(rec.clone())).unwrap();
}
